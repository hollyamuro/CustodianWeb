...
export const loadGroups = () => {
	return async (dispatch)=>{
		try{
			...
			//do request
			const result = 
            await axios.post(location.protocol + "//" + location.host + "/S099C001F004/groups/read", { data: {} } );
...
export const loadAllPermissions = () => {
	return async (dispatch)=>{
		try{
			...
			//do request
			const result = await axios.post(location.protocol + "//" + location.host + "/helper/system_permission_list", {});
...
export const loadGroupPermissions = (groupId) => {
	return async (dispatch)=>{
		try{
			...
			//do request
			const result = await axios.post(location.protocol + "//" + location.host + "/S099C001F004/read", 
...
export const AddPermission = (groupId, sys, dir, fun, permission="") => {
	return async (dispatch)=>{
		try{
			...
			let data = {
				"group_permission_group": groupId,
				"group_permission_sys": sys,
				"group_permission_dir": dir,
				"group_permission_fun": fun,
				"group_permission_auth": permission || Object.keys(PermissionCodes).filter((p)=>{ return (p!=="ALL"); }),
			};

			//do request
			const result = await axios.post(location.protocol + "//" + location.host + "/S099C001F004/create", {"data":data});
	...
	return async (dispatch)=>{
		try{
			...
			let data = {
				...
				"group_permission_dir": dir,
				"group_permission_fun": fun,
				"group_permission_auth": permission,
			};

			//do request
			const result = await axios.post(location.protocol + "//" + location.host + "/S099C001F004/delete", {"data":data});
