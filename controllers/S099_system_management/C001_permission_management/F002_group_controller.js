/**
 * 群組管理
 * @module controllers/S099_system_management/C001_permission_management/F002_group_controller
 */
...
module.exports.init = async (req, res, next) => {
	try {
		...
		// set page
		res.render("S099_system_management/C001_permission_management/F002_group", {
			// default
...
module.exports.read = async (req, res, next) => {
	try {
		...
		if(isInputDataVaild){
			const result = await axios.post(local + "/api/staff/groups/read", { 
				"data": req.body.data, 
...
module.exports.create = async (req, res, next) => {
	try {
		...
		let isInputDataVaild = await utility.checkInputData(req.body.data);
		if(isInputDataVaild){
			const result = await axios.post(local + "/api/staff/groups/create", { 
				"data": req.body.data, 
				"requester": req.user_profile.user,
				"token": req.cookies.custodianweb_access_token,
				"system": "CustodianWeb",
...
module.exports.delete = async (req, res, next) => {
	try {
		...
		let isInputDataVaild = await utility.checkInputData(req.body.data);
		if(isInputDataVaild){
			const result = await axios.post(local + "/api/staff/groups/delete", { 
				"data": req.body.data, 
				"requester": req.user_profile.user,
				"token": req.cookies.custodianweb_access_token,
				"system": "CustodianWeb",
...
module.exports.update = async (req, res, next) => {
	try {
		...
		let isInputDataVaild = await utility.checkInputData(req.body.data);
		if(isInputDataVaild){
			const result = await axios.post(local + "/api/staff/groups/update", { 
				"data": req.body.data, 
				"requester": req.user_profile.user,
				"token": req.cookies.custodianweb_access_token,
				"system": "CustodianWeb",
