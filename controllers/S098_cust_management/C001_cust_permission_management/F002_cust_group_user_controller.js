/**
 * 客戶群組使用者管理Controller
 * @module controllers/S098_cust_management/C001_cust_permission_management/F002_cust_group_user_controller
 */
...
module.exports.init = async (req, res, next) => {
	try{
		...
		// set page
		res.render("S098_cust_management/C001_cust_permission_management/F002_cust_group_user", { 
			// default
...
module.exports.read = async (req, res, next) => {
	try{
		...
		if(isInputDataVaild){
			const result = await axios.post( local + "/api/staff/cust_group_users/read",{
				"data": req.body.data, 
...
module.exports.create = async (req, res, next) => {
	try{
		...
		let isInputDataVaild = await utility.checkInputData(req.body.data);
		if(isInputDataVaild){
			const result = await axios.post( local + "/api/staff/cust_group_users/create",{
				"data": req.body.data, 
				"requester": req.user_profile.user,
				"token": req.cookies.custodianweb_access_token,
				"system": "CustodianWeb",
...
module.exports.delete = async (req, res, next) => {
	try{
		...
		let isInputDataVaild = await utility.checkInputData(req.body.data);
		if(isInputDataVaild){
			const result = await axios.post( local + "/api/staff/cust_group_users/delete",{ 
				"data": req.body.data, 
				"requester": req.user_profile.user,
				"token": req.cookies.custodianweb_access_token,
				"system": "CustodianWeb",
